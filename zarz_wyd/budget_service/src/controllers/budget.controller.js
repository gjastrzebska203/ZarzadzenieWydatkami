...
const getBudgetSummary = async (req, res, next) => {
  try {
    ...
    const error = new Error('Błąd pobierania podsumowania budżetów');
    error.details = err.message;
    next(error);
  }
};
...
const getBudget = async (req, res, next) => {
  try {
    ...
    if (!budget) {
      const error = new Error('Brak budżetu');
      error.status = 404;
      return next(error);
    }
    ...
    await Promise.all(
      activeBudgets.map(async (budget) => {
        try {
          ...
          console.log(`Błąd obsługi budżetu ${budget._id}:`, err.message);
        }
      })
    );
...
const checkBudgetLimits = async (req, res, next) => {
  try {
    ...
    const error = new Error('Błąd sprawdzania limitów');
    error.details = err.message;
    next(error);
  }
};
